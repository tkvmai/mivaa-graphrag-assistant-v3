    ...
    for i, chunk in enumerate(text_chunks):
        ...
        if chunk_results:
            ...
            for item in chunk_results:
                item["chunk"] = i + 1
                item["chunk_text"] = chunk.strip()  # Add actual chunk text

                # print(f"[DEBUG] Chunk {i + 1}: {chunk.strip()[:100]}...")  # Preview first 100 chars
                # print(f"[DEBUG] Triple with chunk text: {item}")
...
def main():
    ...
    parser.add_argument('--debug', action='store_true',
                        help='Enable debug output (raw LLM responses and extracted JSON)')
    ...
    try:
        try:
            ...
            with open(args.input, 'rb') as f:
                ...
                print(f"Warning: Falling back to detected encoding: {encoding}")
                input_text = raw_data.decode(encoding, errors='replace')
    ...
    if result:
        ...
        try:
            ...
            logger.info(f"Saved raw knowledge graph data to {json_output}")

        except Exception as e:
            logger.warning(f"Could not save raw data to {json_output}: {e}")
