...
def visualize_knowledge_graph(triples, output_file="knowledge_graph.html", edge_smooth=None, config=None):
    ...
    options = {
        ...
        "edges": {
            "smooth": {"type": edge_smooth or "dynamic"},
            "color": {"inherit": True},
            "font": {"align": "middle"},
        },
        "interaction": {
        ...
        "physics": {
            "enabled": True,
            "barnesHut": {"gravitationalConstant": -20000, "centralGravity": 0.3, "springLength": 100,
    ...
    # Inject full screen CSS (optional)
    ...
    print(f"[INFO] Graph saved successfully to {output_file}")
...
def _load_html_template():
    """
    Load custom HTML content to inject into the network visualization.
    This can be customized to include dropdowns, sliders, or additional controls.
    """
    return '<div id="mynetwork" class="card-body"></div>'

def _save_and_modify_html(net, output_file, community_count, all_nodes, triples):
    """
    Save the network visualization to an HTML file using PyVis and apply enhancements.

    Args:
        net: PyVis Network object
        output_file: Path to output HTML file
        community_count: Number of detected communities
        all_nodes: List or set of unique node identifiers
        triples: List of SPO triples in the graph
    """
    try:
        # Use PyVis to generate HTML content in memory (avoids encoding errors)
        html = net.generate_html()

        # Replace PyVis placeholder network div with a custom layout (if needed)
        html = html.replace('<div id="mynetwork" class="card-body"></div>', _load_html_template())

        # Remove duplicate or default title
        html = re.sub(r'<center>\s*<h1>.*?</h1>\s*</center>', '', html)

        # Add custom title with graph stats
        ...
        # Write the final HTML with UTF-8 encoding
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html)
        ...
        logging.error("Failed to save and modify HTML: %s", e, exc_info=True)
